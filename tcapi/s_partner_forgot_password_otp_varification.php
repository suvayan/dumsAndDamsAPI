<?php
    ...
    function deleteTempRow($mysqli, $rid, $otp){
        $sql = "delete from temp_partner_forgot_passwor_otp_tbl where id='{$rid}' and otp='{$otp}'";
    ...
    if(count($required) > 0){
        ...
        $queryOne = "select * from temp_partner_forgot_passwor_otp_tbl where id='{$rid}' and otp='{$otp}'";
        ...
        if($resultOne->num_rows && $tempRowDelete){
            ...
            $email    = $row['email'];
            $setNewPassword = $mysqli->query("update partner set password='{$password}' where email='{$email}'");
            if($setNewPassword){
